...
public class CellMummyMixin extends TheNecoraIC implements IRelicItem {
    ...
    public RelicData constructDefaultRelicData() {
        return RelicData.builder()
                .abilities(AbilitiesData.builder().ability(AbilityData.builder(CellMummy.doas)
                        .stat(StatData.builder(CellMummy.stat)
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        if (stack.getItem() instanceof IRelicItem relicItem) {
            if (slotContext.entity() instanceof Player player) {
                ...
                int range = 6;
                List<cell_zombie> entities = player.level().getEntitiesOfClass(cell_zombie.class, new AABB(playerPos.x - (double) range, playerPos.y - (double) range, playerPos.z - (double) range, playerPos.x + (double) range, playerPos.y + (double) range, playerPos.z + (double) range));
                for (cell_zombie cellZombie : entities) {
                    if (cellZombie.getOwner() != null && cellZombie.getOwner().is(player)) {
                        if (player.tickCount % 20 == 1) {
                            ...
                            cellZombie.time -= (int) relicItem.getStatValue(stack,CellMummy.doas,CellMummy.stat) * 20;
